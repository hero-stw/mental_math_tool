...
function cal3() {
  function genAddend() {
    ...
    for (i = 0; i < 5; i++) {
      ...
      operationText[i] = `${numDot(addend[i])}USD to EUR`;
    }
    // Generate curency exchange Euro => $
    for (var j = 5; j < 10; j++) {
      operation[j] = roundto2(addend[j] / euro);
      operationText[j] = `${numDot(addend[j])}EUR to USD`;
    }
    for (i = 0; i < 5; i++) {
  ...
  answer[4].addEventListener("keyup", function (event) {
    if (event.keyCode === 13) {
      ...
      if (!check(delta[4])) {
        ...
        margin_arr[4].innerHTML = delta[4] + " %";
...
function next(no) {
  ...
  margin_arr[no].innerHTML = delta[no] + " %";
...
function countDelta() {
  for (var i = 0; i < 5; i++) {
    var d = (Math.abs(answer[i].value - operation[i]) / operation[i]) * 100;
    if (d > 100) delta[i] = 100;
...
var newPlr = new Player();

function save() {
  var input = document.getElementById("name-plr").value;
...
function Player(name, error, duration, timenum) {
  this.name = name;
  this.errs = Number(error);
  this.durr = duration;
  this.timenum = timenum;
}
var plrname = document.querySelectorAll("#basic .plr-name");
var timemax;
var def = new Player("Player", "0.0", "00s 00", 10000);
var p1 = new Player("Player 1", "2.03", "5s 00", 500);
// var p2 = new Player("Player 2", "40.03", "4s 00", 400);
// var p3 = new Player("Player 3", "50.03", "7s 00", 700);
var basicres = [def, def, def, def, def, def, def, def, def, def];
var addres = [def, def, def, def, def, def, def, def, def, def];
var subres = [def, def, def, def, def, def, def, def, def, def];
var mulres = [def, def, def, def, def, def, def, def, def, def];
var divres = [def, def, def, def, p1, def, def, def, def, def];
var perres = [def, def, def, def, def, def, def, def, def, def];
var per1res = [def, def, def, def, def, def, def, def, def, def];
var per2res = [def, def, def, def, def, def, def, def, def, def];
var cures = [def, def, def, def, def, def, def, def, def, def];
...
function onboard(a, array) {
  for (var i = 0; i <= 9; i++) {
    a[i].innerHTML = array[i].name;
    a[i].nextSibling.nextSibling.childNodes[1].innerHTML = array[i].errs + "%";
    a[i].nextSibling.nextSibling.childNodes[3].innerHTML = array[i].durr;
  }
}
sort(basicres);
onboard(plrname, basicres);
...
function resetVal() {
  ...
  for (var i = 0; i <= 4; i++) {
    ...
    reslog[i].innerHTML = "";
    answer[i].removeAttribute("readonly");
...
class Stopwatch {
  ...
  lap(n) {
    ...
    if (n == 0) {
      ...
      time_lap[n] = msToHMS(pad0(time_count[n], 2));
    }
    console.log("Laps: " + timeTemp);
  ...
  beforeStop(n) {
    ...
    console.log("Stop laps: " + timeTemp);
    console.log(time_lap);
    console.log(time_count);
  }
  ...
  format(times) {
    ...
    //   }
    return `\ ${pad0(times[1], 2)}s\ ${pad0(Math.floor(times[2]), 2)}`;
  }
}
...
function changebtn() {
  ...
  resetVal();
  stopwatch.clear();
  document.getElementById("ldbname").innerHTML =
    document.getElementsByTagName("option")[choice - 1].innerHTML;
  sort(resarr[choice]);
  onboard(plrname, resarr[choice]);
}
// Show result
function showRes(n) {
  var sum = 0;
  ...
  function printRes() {
    ...
    console.log(n);
    console.log("Tongtgian: " + sum);
    console.log(sum / n);
    avg = roundto2(sum / n);
    avgdel = roundto2(sumdel / n);
    document.getElementById("res1").innerHTML = avgdel;
    document.getElementById("res2").innerHTML = msToHMS(avg);
  }
  if (n == 0) {
    document.getElementById("res1").innerHTML = delta[0];
